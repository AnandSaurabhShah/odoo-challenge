...
export default function App() {
  // 1. Client State (Authentication & Navigation)
  const currentUser = useAssetStore((state) => state.currentUser);
  const activeScreen = useAssetStore((state) => state.activeScreen);
  const setActiveScreen = useAssetStore((state) => state.setActiveScreen);
  const handleLogin = useAssetStore((state) => state.handleLogin);
  const handleLogout = useAssetStore((state) => state.handleLogout);

  // 2. Server State (Fetched & Cached via TanStack Query)
  ...
  // 3. Server Actions (Executed via TanStack Query Mutations)
  ...
  const handleRequestTransfer = (newTrans) =>
    requestTransferMutation.mutate(newTrans);
  const handleApproveTransfer = (transId) =>
    approveTransferMutation.mutate(transId);
  const handleRejectTransfer = (transId) =>
    rejectTransferMutation.mutate(transId);
  ...
  const handleAddMaintenanceTask = (newTask) =>
    addMaintenanceTaskMutation.mutate(newTask);
  ...
  const handleQuickAction = (screenId) => {
  ...
  // Projection / Selectors Layer to match exact frontend component prop requirements
  ...
  const departmentsMapped = departments.map((d) => {
    ...
    const totalVal = deptAssets.reduce((sum, a) => sum + a.acquisitionCost, 0);
  ...
  const assetsMapped = assets.map((a) => {
    ...
    // Find active allocation (returnedAt is null)
    ...
    else if (a.status === AssetStatus.UNDER_MAINTENANCE)
      uiStatus = "In Maintenance";
  ...
  const maintenanceMapped = maintenance.map((m) => {
    ...
    else if (m.status === MaintenanceStatus.ASSIGNED)
      uiStatus = "Technician Assigned";
  ...
  const auditsMapped = audits.map((aud) => {
    ...
    const discrepancies = discItems.map((item) => {
      ...
      if (item.status === AuditItemStatus.DAMAGED) {
        issue =
          item.remarks || "Hardware reported damaged during audit cycle scan.";
  ...
  const logsMapped = logs.map((l) => {
    ...
    else if (l.entityType === EntityType.MAINTENANCE)
        ...
        {isQueryLoading ? (
          <div className="flex flex-col items-center justify-center min-h-[70vh] space-y-4">
            <div className="relative flex items-center justify-center">
              ...
              <div className="absolute w-6 h-6 border-4 border-emerald-500/10 border-b-emerald-500 rounded-full animate-spin animation-reverse"></div>
            </div>
            <div className="text-center space-y-1">
              <p className="text-slate-300 font-medium text-sm tracking-wide">
                Syncing Register Registry
              </p>
